...
int lowestIdle()
{
	...
	for (i=0; i<users; i++)
	{
		...
		/* COnverts seconds.milliseconds to secs */	
		if (index(buff, '.') != NULL)
		{
			int temp2 = 0; 
			num = strtok(buff, ".");
	...
	while(1)
	{
		...
		while(ftell(fp) != maxFileSize)
		{
			...
			{	
				...
				printf("num of pr lnotpp %d\n", numProcesses);

			if(ftell(fp) == 0)
			printf("DIEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE\n");
		}
		printf("num of main looppr %d\n", numProcesses);
	}
}
